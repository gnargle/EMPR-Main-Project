...
def transform():
	global minimum1, minimum2, minimum3, minimum4, flipped1, flipped2, flipped3, flipped4
	for val in flipped1:
		optimusprime = abs(val[1] * math.sin(math.radians(val[0])))
		if optimusprime < minimum1:
			minimum1 = optimusprime
	for val in flipped2:
		optimusprime = abs(val[1] * math.cos(math.radians(val[0])))
		if optimusprime < minimum2:
			minimum2 = optimusprime
	for val in flipped3:
		optimusprime = abs(val[1] * math.sin(math.radians(val[0])))
		if optimusprime < minimum3:
			minimum3 = optimusprime
	for val in flipped4:
		optimusprime = abs(val[1] * math.cos(math.radians(val[0])))
		if optimusprime < minimum4:
			minimum4 = optimusprime
	screen.fill([0,0,0]) 
	pygame.display.set_caption("Multiview Mode")
	multi_buttons()
	pygame.draw.line(screen, (0,51,102), (400,40), (400,360),3)
