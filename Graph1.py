...
def draw_tapemeasureplot():  #tape measure mode 
	...
	y = 10
	draw_buttons()
	if len(calpoints) == 3: #remove points if calpoints == 0? all of them do. before and after calibration?
		calpoints.pop(0)
...
def draw_overheadplot(): #overhead plot
	global us, ir,ir_raw,us_raw, a, sensor, screen, pointlist, no_points,previousmode, previous# need to set points list to zero after mode changes
...
def draw_radarplot(): #radar mode
	global angle, max_rot, min_rot, us, ir, us_raw, ir_raw, sensor, mode, rpointlist,previousmode, next, angledict, lastmax, lastmin, previous
	screen.fill([0,0,0]) #clear point list
...
def draw_multiviewplot(): #multiview mode '''maybe plot all the sweeps at the same time'''
	global mode, sensor, ir, us, us_raw, ir_raw, max_rot, min_rot, sweep_num, angle, p0, p1, p2, p3, mcount, previous
	screen.fill([0,0,0]) #clear point list
	if previous == "multi":
		pass
	else:
	...
	if sweep_num == 0 and mcount == 1: #180 -> 0 #need to do angle bounds
		p0 = []
		p1 = []
		p2 = []
		p3 = []
...
def endofsweep():
	global sweep_num, lastsweep, pointlist, rpointlist, a
